...
public class ExportReportService {
    ...
    public ExportReportService(JavaMailSender javaMailSender, MonthlyReportsRepository monthlyReportsRepository,
                               RequestStateManager requestStateManager, YearlyReportsRepository yearlyReportsRepository){
    ...
    private void sendFileEmail(ByteArrayResource file, String email, String name, int year ){
        try{
            ...
            MimeMessageHelper helper = new MimeMessageHelper(message, true);
            helper.setTo(email);
            helper.setSubject(mailText.getExportReportYearlyMailSubject().replace("${year}", String.valueOf(year)));
            helper.setText(mailText.getExportReportYearlyMail().replace("${year}", String.valueOf(year)));
            helper.addAttachment(name+".xlsx", file);
            javaMailSender.send(message);
        }catch (Exception e){
            System.out.println(e.getMessage());
    ...
    public void exportYearlyRecords(YearlyReportFetchDto yearlyReportFetchDto, User user, String email) {
        Optional<List<MonthlyReportPublicDto>> optionalList = yearlyReportsRepository.getYearlyRecordOfUser(user.getUserId(), yearlyReportFetchDto.getYReportYear());
        if(optionalList.isEmpty())return;
        ...
        double totalAmount=0;
        try(Workbook workbook = new XSSFWorkbook()){
            String name=String.format("Yearly Report %d", yearlyReportFetchDto.getYReportYear());
            Sheet sheet = workbook.createSheet(name);
            Row headerRow=sheet.createRow(0);
            for(int i=0;i<headerLength;i++){
                headerRow.createCell(i).setCellValue(headers[i]);
            ...
            for(int i=1;i<=list.size();i++){
                ...
                dataRow.createCell(2).setCellValue(list.get(i-1).getMReportNarration());
                totalAmount+=list.get(i-1).getMReportAmount();
                requestStateManager.setExportingYearlyDataState(user, (i < size), (i*100)/size);
            }
            Row lastRow=sheet.createRow(list.size()+2);
            lastRow.createCell(1).setCellValue("Collected: ₹"+totalAmount);
            for (int i = 0; i < headerLength; i++) {
            ...
            sendFileEmail(
                    ...
                    email,
                    name,
                    yearlyReportFetchDto.getYReportYear()
            );
        }catch (Exception e){
            System.out.println(e.getMessage());
        }
